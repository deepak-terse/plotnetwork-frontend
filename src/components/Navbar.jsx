...
class Navbar extends Component {
  ...
  render () {
    const fullName = JSON.parse(localStorage.getItem('loggedInUser')).fullName;
    const { logoShort } = JSON.parse(localStorage.getItem('loggedInUser')).partnerDetails;

    return (
      <nav className="navbar p-0 fixed-top d-flex flex-row">
        <div className="navbar-brand-wrapper d-flex d-lg-none align-items-center justify-content-center">
          <Link className="navbar-brand brand-logo-mini" to="/"><img src={logoShort} alt="logo" /></Link>
        </div>
          ...
          <ul className="navbar-nav navbar-nav-right">
            <Dropdown alignRight as="li" className="nav-item border-left">
              ...
              <Dropdown.Menu className="dropdown-menu navbar-dropdown preview-list">
                <p className="p-3 mb-0 text-center"><Trans>No new notifications</Trans></p>
              </Dropdown.Menu>
            </Dropdown>
